...
kappa_G_z = np.array(curvature.kappa_G)

# Compute the partial derivative in the direction of offset
t = time.time()
curvature.reverse()
print(f'Time to compute reverse: {time.time() - t}')
dif_kappa_G = np.zeros(kappa_G_z.shape)
for i in range(len(dif_kappa_G)):
    for j, d in curvature.dif_kappa_G[i].items():
        dif_kappa_G[i] += d * dz[j]

# Estimate the partial derivative by adding, evaluating, and subtracting
mesh.set_parameters(z + dz)
curvature.forward()
kappa_G_z_dz = np.array(curvature.kappa_G)
estimated_dif_kappa_G = kappa_G_z_dz - kappa_G_z

# Print something close to 1., hopefully
quotient = np.linalg.norm(dif_kappa_G) / np.linalg.norm(estimated_dif_kappa_G)
print(f'Quotient of magnitudes: {quotient:.6f}')
# Print something close to 0., hopefully
angle = np.arccos(dif_kappa_G @ estimated_dif_kappa_G
                  / (np.linalg.norm(dif_kappa_G)
                     * np.linalg.norm(estimated_dif_kappa_G)))
