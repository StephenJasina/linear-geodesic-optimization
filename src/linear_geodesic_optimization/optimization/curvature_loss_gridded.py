...
class Computer:
    ...
    def __init__(
        ...
        self._weight_grid = np.zeros((mesh.get_topology().n_vertices))
        for fat_edge, edge_weight in zip(self._fat_edges, self._edge_weights):
            for vertex in fat_edge:
                self._weight_grid[vertex.index] += edge_weight
    ...
    def forward(self) -> None:
        ...
        * `Computer.loss`
        ...
        for fat_edge, network_curvature, edge_weight in zip(
            ...
            for vertex in fat_edge:
                loss_part += (self._curvature.kappa_G[vertex.index] - network_curvature)**2 / self._weight_grid[vertex.index]
        ...
        for fat_edge, network_curvature, edge_weight \
            ...
            for vertex in fat_edge:
                ...
                for index, partial \
                    ...
                    self.dif_loss[index] \
                        += 2 * edge_weight * (curvature - network_curvature) * partial / self._weight_grid[vertex.index] / len(fat_edge)
