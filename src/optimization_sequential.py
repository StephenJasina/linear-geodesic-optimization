...
def main(
    *,  # All parameters are keyword only
    ...
    sphere_cap = sphere_cap - np.amin(sphere_cap)
...
if __name__ == '__main__':
    directory_outputs = pathlib.PurePath('..', 'outputs', 'toy', 'parallel_links')

    epsilon = None
    directory_json = pathlib.PurePath('toy', 'routing_with_volumes', 'epsilon_changes', 'parallel_links')
    ...
    initialization_file_paths = [None] + [directory_output / 'output.json' for directory_output in directories_output[:-1]]
    ...
    for (
        ...
        directory_output, maxiter, initialization_file_path
    ) in zip(
        ...
        directories_output, maxiters, initialization_file_paths
    ):
        main(
            ...
            maxiter = maxiter,
            initialization_file_path = initialization_file_path
        )
