...
function setCanvasZoom(newCanvasResolution) {
  textureBlank.dispose();
  ctxBlank.canvas.width = newCanvasResolution;
  ctxBlank.canvas.height = newCanvasResolution;
...
function makeTextureMap() {
  ...
  let textureMap = new THREE.CanvasTexture(ctxMap.canvas);
...
function setMapZoom(newCanvasResolution) {
  if (textureMap != null) {
    textureMap.dispose();
  }
  ...
  divMap.style.width = (newCanvasResolution / window.devicePixelRatio) + "px";
  divMap.style.height = (newCanvasResolution / window.devicePixelRatio) + "px";
  olMap.updateSize();
  olMap.getView().setCenter(fromLonLat(mapCenter));
  olMap.getView().setResolution(CIRCUMFERENCE_EARTH / newCanvasResolution / mapZoomFactor * window.devicePixelRatio);
...
function wheelEvent(event) {
  ...
  let newMapResolution = canvasResolution * Math.min(camera.zoom, 4.);
  if (checkboxShowMap.checked) {
    setMapZoom(newMapResolution);
  } else {
    setCanvasZoom(newMapResolution);
  }
  ...
  for (let i = 0; i < divisions; i++) {
    for (let j = 0; j < divisions; j++) {
      if (z_left[i][j] == null || z_right[i][j] == null) {
        ...
        bufferedHeights.setZ(i * divisions + j, ((1 - lambda) * z_left[i][j] + lambda * z_right[i][j]) * 3.);
      }
    }
  }
...
function drawHoverGraph(index) {
  ...
  // TODO: Better coloration here
  let edges = new LineSegments2(edgesGeometry, new LineMaterial({
    color: COLOR_EDGE,
