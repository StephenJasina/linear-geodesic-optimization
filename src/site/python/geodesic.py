...
from python.util import veclen, normalized
'''
This is a class for computing geodesic distances on triangle meshes. The class has the following methods:

__init__(self, verts, tris, m=10.0): This method initializes the class with the vertices and triangles of the mesh and a parameter m that determines the step size for the heat method used to compute the distances. It also pre-computes some values that are needed later on.

__call__(self, idx): This method computes the geodesic distances to all vertices in the mesh from the vertex specified by idx. idx can be either a single vertex index, a list of vertex indices, or an array of bools with length equal to the number of vertices in the mesh.

To use the class, you first need to create an instance of it with the vertices and triangles of the mesh, and then call the instance with a vertex index to compute the distances to all vertices from that point. For example:
